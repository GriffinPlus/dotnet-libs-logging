...
partial class LogFile
{
	...
	private sealed class AnalysisDatabaseAccessor : DatabaseAccessor
	{
		...
		private readonly struct PruneOperationState1
		{
			...
			public required AnalysisDatabaseAccessor Accessor                { get; init; }
		}
		...
		public override int Prune(long maximumMessageCount, DateTime minimumMessageTimestamp)
		{
			...
			static int Operation(PruneOperationState1 state)
			{
				...
				if (messageId >= 0)
				{
					...
					state.Accessor.mDeleteTextsUpToIdCommand_IdParameter.Value = messageId;
					ExecuteNonQueryCommand(state.Accessor.mDeleteTextsUpToIdCommand);
