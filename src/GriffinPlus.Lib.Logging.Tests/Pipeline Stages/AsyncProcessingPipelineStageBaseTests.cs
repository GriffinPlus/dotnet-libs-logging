...
namespace GriffinPlus.Lib.Logging
{
	...
	public abstract class AsyncProcessingPipelineStageBaseTests<STAGE> where STAGE: AsyncProcessingPipelineStage<STAGE>
	{
		...
		/// <summary>
		/// Test whether initializing the pipeline stage using <see cref="AsyncProcessingPipelineStage{STAGE}.Initialize"/> throws an exception,
		...
		/// <summary>
		/// Tests whether shutting down the pipeline stage using <see cref="AsyncProcessingPipelineStage{STAGE}.Shutdown"/> succeeds,
		...
		{
			var stage1 = CreateStage();
			var stage2 = new AsyncProcessingPipelineTestStage();
			stage1.AddNextStage(stage2);
		...
		/// <summary>
		/// Test whether <see cref="AsyncProcessingPipelineStage{STAGE}.Process(LocalLogMessage)"/> throws an exception,
