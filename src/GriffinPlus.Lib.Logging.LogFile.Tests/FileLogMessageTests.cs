...
public class FileLogMessageTests : IDisposable
{
	...
	/// <see langword="true"/> to register the <see cref="INotifyPropertyChanged.PropertyChanged"/> event and check whether it is fired correctly;<br/>
	...
	/// <summary>
	/// Test data for message property checks (min/max values and protection flag).
	/// </summary>
	public static TheoryData<Expression<Func<LogFileMessage, object>>, object, object, bool> MessagePropertyTestData
	{
		get
		{
			var data = new TheoryData<Expression<Func<LogFileMessage, object>>, object, object, bool>();

			foreach (bool protect in new[] { false, true })
			{
				data.Add(EXPR<LogFileMessage, object>(x => x.Id), -1L, 1L, protect);
				data.Add(EXPR<LogFileMessage, object>(x => x.LostMessageCount), 0, 1, protect);
				data.Add(EXPR<LogFileMessage, object>(x => x.Timestamp), default(DateTimeOffset), DateTimeOffset.Now, protect);
				data.Add(EXPR<LogFileMessage, object>(x => x.HighPrecisionTimestamp), 0L, 1L, protect);
				data.Add(EXPR<LogFileMessage, object>(x => x.LogWriterName), null, "A Log Writer", protect);
				data.Add(EXPR<LogFileMessage, object>(x => x.LogLevelName), null, "A Log Level", protect);
				data.Add(EXPR<LogFileMessage, object>(x => x.Tags), null, new TagSet("Tag"), protect);
				data.Add(EXPR<LogFileMessage, object>(x => x.ApplicationName), null, "Application Name", protect);
				data.Add(EXPR<LogFileMessage, object>(x => x.ProcessName), null, "Process Name", protect);
				data.Add(EXPR<LogFileMessage, object>(x => x.ProcessId), -1, 1, protect);
				data.Add(EXPR<LogFileMessage, object>(x => x.Text), null, "Some Text", protect);
			}
	...
	/// <summary>
	/// Test data for equality checks of <see cref="LogFileMessage"/> properties.
	/// </summary>
	public static TheoryData<LogFileMessage, Expression<Func<LogFileMessage, object>>, object> EqualityTestData
	{
		get
		{
			var data = new TheoryData<LogFileMessage, Expression<Func<LogFileMessage, object>>, object>();
			...
			data.Add(message, EXPR<LogFileMessage, object>(x => x.Id), -10L);
			data.Add(message, EXPR<LogFileMessage, object>(x => x.Id), 11L);
			data.Add(message, EXPR<LogFileMessage, object>(x => x.LostMessageCount), -10);
			data.Add(message, EXPR<LogFileMessage, object>(x => x.LostMessageCount), 10);
			data.Add(message, EXPR<LogFileMessage, object>(x => x.Timestamp), DateTimeOffset.Parse("2020-01-02T12:00:00+01:00"));
			data.Add(message, EXPR<LogFileMessage, object>(x => x.HighPrecisionTimestamp), -10L);
			data.Add(message, EXPR<LogFileMessage, object>(x => x.HighPrecisionTimestamp), 10L);
			data.Add(message, EXPR<LogFileMessage, object>(x => x.LogWriterName), "Another Log Writer");
			data.Add(message, EXPR<LogFileMessage, object>(x => x.LogWriterName), null);
			data.Add(message, EXPR<LogFileMessage, object>(x => x.LogLevelName), "Another Log Level");
			data.Add(message, EXPR<LogFileMessage, object>(x => x.LogLevelName), null);
			data.Add(message, EXPR<LogFileMessage, object>(x => x.Tags), new TagSet("AnotherTag"));
			data.Add(message, EXPR<LogFileMessage, object>(x => x.Tags), null);
			data.Add(message, EXPR<LogFileMessage, object>(x => x.ApplicationName), "Another Application");
			data.Add(message, EXPR<LogFileMessage, object>(x => x.ApplicationName), null);
			data.Add(message, EXPR<LogFileMessage, object>(x => x.ProcessName), "Another Process Name");
			data.Add(message, EXPR<LogFileMessage, object>(x => x.ProcessName), null);
			data.Add(message, EXPR<LogFileMessage, object>(x => x.ProcessId), -10);
			data.Add(message, EXPR<LogFileMessage, object>(x => x.ProcessId), 10);
			data.Add(message, EXPR<LogFileMessage, object>(x => x.Text), "Some other Text");
			data.Add(message, EXPR<LogFileMessage, object>(x => x.Text), null);
