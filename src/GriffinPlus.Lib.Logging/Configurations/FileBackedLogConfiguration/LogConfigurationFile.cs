...
public class LogConfigurationFile
{
	...
	private static readonly Regex sSectionRegex = new(
		@"^\s*\[([-_:\. \w]+)\](.*)",
		RegexOptions.Compiled);

	private static readonly Regex sKeyValueRegex = new(
		@"^\s*(\w[-_\. \w]+?)\s*=\s*([^;]*?)\s*(;.*)?$",
		RegexOptions.Compiled);

	private static readonly Regex sProcessingPipelineStageSectionRegex = new(
