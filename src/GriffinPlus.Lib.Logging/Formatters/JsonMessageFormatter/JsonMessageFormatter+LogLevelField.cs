...
namespace GriffinPlus.Lib.Logging
{
	partial class JsonMessageFormatter
	{
		...
		sealed class LogLevelField : FieldBase
		{
			...
			public LogLevelField(JsonMessageFormatter formatter, string jsonKey) : base(formatter, jsonKey)
			{
			...
			{
				builder.Append('"');
				if (message.LogLevelName != null) AppendEscapedStringToBuilder(builder, message.LogLevelName);
				builder.Append('"');
			}
		}
	}
