...
namespace GriffinPlus.Lib.Logging
{
	partial class JsonMessageFormatter
	{
		...
		sealed class ApplicationNameField : FieldBase
		{
			...
			public ApplicationNameField(JsonMessageFormatter formatter, string jsonKey) : base(formatter, jsonKey)
			{
			...
			{
				builder.Append('"');
				if (message.ApplicationName != null) AppendEscapedStringToBuilder(builder, message.ApplicationName);
				builder.Append('"');
			}
		}
	}
