...
namespace GriffinPlus.Lib.Logging
{
	partial class JsonMessageFormatter
	{
		...
		sealed class ProcessNameField : FieldBase
		{
			...
			public ProcessNameField(JsonMessageFormatter formatter, string jsonKey) : base(formatter, jsonKey)
			{
			...
			{
				builder.Append('"');
				if (message.ProcessName != null) AppendEscapedStringToBuilder(builder, message.ProcessName);
				builder.Append('"');
			}
		}
	}
